...
class _MyAppState extends State<MyApp> {
  ...
  String _scanBtnTxt = "startScan()";
  String _scanBtnTxt15 = "startScan(15)";
        ...
        body: Center(
          child: Column(
            children: <Widget>[
              ...
              ElevatedButton(
                  child: Text(_scanBtnTxt15),
                  onPressed: () =>onStartScanWithPeriod(15*1000)),
              ElevatedButton(
  ...
  void onStartScan() {
    if (!mounted) return;
    _blePlugin.startScan.then((value) => {
      setState(() {
  ...
  Future<void> onStartScanWithPeriod(int scanPeriod) async {
    if (!mounted) return;
    _blePlugin.startScanWithPeriod(scanPeriod).then((value) => {
      setState(() {
        _scanBtnTxt15 = value ? "Scanning" : "Scan filed";
      })
    }).onError((error, stackTrace) => {
